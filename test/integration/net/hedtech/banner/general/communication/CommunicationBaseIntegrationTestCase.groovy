    ...
    protected void deleteAll() {
        ...
        try {
            sessionFactory.currentSession.with { session ->
                ...
                sql.executeUpdate("Delete from GCBEVMP where gcbevmp_system_ind = 'N'")
                ...
                sql.executeUpdate("Delete from GCBEMTL WHERE EXISTS (SELECT gcbtmpl_surrogate_id FROM gcbtmpl WHERE gcbtmpl_surrogate_id = gcbemtl_surrogate_id AND gcbtmpl_system_ind = 'N')")
                sql.executeUpdate("Delete from GCBMNTL")
                sql.executeUpdate("Delete from GCBLTPL")
                sql.executeUpdate("Delete from GCBTMPL  WHERE gcbtmpl_system_ind = 'N'")
